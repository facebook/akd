...
/// NodeLabel.val is a u64 at the moment, but eventually
/// will be migrated for a [u8; 32] with the addition of VRFs.
/// This messaging logic assumes it's already a byte array
/// for forward compat.
const NODE_LABEL_VALUE_BYTES: usize = 8;
...
impl TryFrom<akd::node_state::NodeLabel> for inter_node::NodeLabel {
    ...
    fn try_from(input: akd::node_state::NodeLabel) -> Result<Self, Self::Error> {
        ...
        result.set_val(input.val.to_be_bytes().to_vec());
        Ok(result)
    }
}
...
impl TryFrom<&inter_node::NodeLabel> for akd::node_state::NodeLabel {
    ...
    fn try_from(input: &inter_node::NodeLabel) -> Result<Self, Self::Error> {
        ...
        let val_bytes_vec = input.get_val();
        let val_bytes_len = val_bytes_vec.len();
        let val_bytes: [u8; NODE_LABEL_VALUE_BYTES] = input.get_val().try_into().map_err(|_| {
            ConversionError::Serialization(format!(
                "NodeLabel value bytes is of incorrect length. Expected {}, received {}",
                NODE_LABEL_VALUE_BYTES, val_bytes_len
            ))
        })?;

        Ok(akd::node_state::NodeLabel {
            len: input.get_len(),
            val: u64::from_be_bytes(val_bytes),
        })
    }
}
