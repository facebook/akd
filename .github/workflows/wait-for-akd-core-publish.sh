...
akd_version_expected=$(get_crate_expected_version_number "akd" "akd_core")
echo "AKD_CORE expected version in AKD:" $akd_version_expected
wait_time=1
while sleep $wait_time;
do
    akd_versions_published=$(
        get_crate_published_versions "https://raw.githubusercontent.com/rust-lang/crates.io-index/master/3/a/akd_core"
    )
    echo "AKD_CORE published versions:" $akd_versions_published
