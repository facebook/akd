...
const SELECT_HISTORY_TREE_NODE_DATA: &str =
    "`location`, `label_len`, `label_val`, `birth_epoch`, `last_epoch`, `parent`, `node_type`";
...
impl<'a> AsyncMySqlDatabase {
    ...
    async fn setup_database(mut conn: mysql_async::Conn) -> core::result::Result<(), MySqlError> {
        ...
        let command = "CREATE TABLE IF NOT EXISTS `".to_owned()
            ...
            + " `label_val` BIGINT UNSIGNED NOT NULL, `birth_epoch` BIGINT UNSIGNED NOT NULL,"
...
impl MySqlStorable for DbRecord {
    fn set_statement(&self) -> String {
        match &self {
            ...
            DbRecord::HistoryTreeNode(_) => format!("INSERT INTO `{}` ({}) VALUES (:location, :label_len, :label_val, :birth_epoch, :last_epoch, :parent, :node_type) ON DUPLICATE KEY UPDATE `label_len` = :label_len, `label_val` = :label_val, `birth_epoch` = :birth_epoch, `last_epoch` = :last_epoch, `parent` = :parent, `node_type` = :node_type", TABLE_HISTORY_TREE_NODES, SELECT_HISTORY_TREE_NODE_DATA),
    ...
    fn set_params(&self) -> mysql_async::Params {
        match &self {
            ...
            DbRecord::HistoryTreeNode(node) => {
                params! { "location" => node.location, "label_len" => node.label.len, "label_val" => node.label.val, "birth_epoch" => node.birth_epoch, "last_epoch" => node.last_epoch, "parent" => node.parent, "node_type" => node.node_type as u8 }
            }
            DbRecord::ValueState(state) => {
    ...
    fn set_batch_statement<St: Storable>(items: usize) -> String {
        ...
        for i in 0..items {
            match St::data_type() {
                ...
                StorageType::HistoryTreeNode => {
                    parts = format!("{}(:location{}, :label_len{}, :label_val{}, :birth_epoch{}, :last_epoch{}, :parent{}, :node_type{})", parts, i, i, i, i, i, i, i);
                }
        ...
        match St::data_type() {
            ...
            StorageType::HistoryTreeNode => format!("INSERT INTO `{}` ({}) VALUES {} as new ON DUPLICATE KEY UPDATE `label_len` = new.label_len, `label_val` = new.label_val, `birth_epoch` = new.birth_epoch, `last_epoch` = new.last_epoch, `parent` = new.parent, `node_type` = new.node_type", TABLE_HISTORY_TREE_NODES, SELECT_HISTORY_TREE_NODE_DATA, parts),
    ...
    fn get_batch_statement<St: Storable>() -> String {
        match St::data_type() {
            ...
            StorageType::HistoryTreeNode => {
                format!(
                    "SELECT a.`location`, a.`label_len`, a.`label_val`, a.`birth_epoch`, a.`last_epoch`, a.`parent`, a.`node_type` FROM `{}` a INNER JOIN {} ids ON ids.`location` = a.`location`",
                    TABLE_HISTORY_TREE_NODES,
                    TEMP_IDS_TABLE
                )
            }
