...
use super::{Message, DEFAULT_FONT_SIZE, DEFAULT_SPACING};
use crate::storage::dynamodb::DynamoDbClapSettings;
use crate::storage::s3::S3ClapSettings;
use crate::storage::CommonStorageClapSettings;
...
impl CommonStorageClapSettings for StorageSettings {
...
pub enum SettingsMessage {
    ...
    Loaded(Result<StorageSettings, saved_state::LoadError>),
        ...
        match settings_msg {
            ...
            SettingsMessage::CancelSettings => {
                ...
                Command::perform(async {}, |_| {
                    Message::TabSelected(super::AUDITOR_INTERFACE_TAB)
                })
            }
            SettingsMessage::S3EndpointChanged((endpoint, endpoint_value)) => {
                if endpoint {
                    self.set_s3_endpoint(Some(endpoint_value));
                } else {
                    self.set_s3_endpoint(None);
                }
                Command::none()
            }
            SettingsMessage::AccessKeyChanged((key, key_value)) => {
                if key {
                    self.set_access_key(Some(key_value));
                } else {
                    self.set_access_key(None);
                }
                Command::none()
            }
            SettingsMessage::SecretKeyChanged((key, key_value)) => {
                if key {
                    self.set_secret_key(Some(key_value));
                } else {
            ...
            SettingsMessage::DynamoEndpointChanged((endpoint, endpoint_value)) => {
                if endpoint {
                    self.set_dynamo_endpoint(Some(endpoint_value));
                } else {
                    self.set_dynamo_endpoint(None);
                }
                Command::none()
            }
            ...
            SettingsMessage::Saved(save_result) => match save_result {
                Ok(_) => {
                    log::info!("Settings saved.");
                    Command::batch([
                        Command::perform(async {}, |_| Message::SettingsUpdated),
...
impl super::AuditorTab for StorageSettings {
    ...
    fn view(&self) -> Element<Message> {
        let settings_view = match &self {
            StorageSettings::S3(s3_settings) => Self::s3_view(s3_settings),
            StorageSettings::DynamoDb(dynamo_settings) => Self::dynamo_view(dynamo_settings),
        };
                    ...
                    row.push(radio(
                        ...
                        *ty,
                        Some(match self {
                            ...
                            Self::DynamoDb(_) => StorageType::DynamoDb,
                        }),
