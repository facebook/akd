...
type InMemoryDb = seemless::storage::memory::InMemoryDatabase;
...
fn single_insertion(c: &mut Criterion) {
    ...
    let mut azks1 = Azks::<Blake3, InMemoryDb>::new(&db, &mut rng).unwrap();
    ...
    for _ in 0..num_nodes {
        ...
        azks1.insert_leaf(&db, node, val).unwrap();
    }
